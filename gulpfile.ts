...
gulp.task(
	'coveralls',
	gulp.series('test', shell.task('nyc report --reporter=text-lcov'))
)

gulp.task('lint', shell.task('eslint --fix' + files.join(' ')))

gulp.task('format', shell.task('prettier --write ' + files.join(' ')))

gulp.task('default', gulp.series('build', 'test', 'coverage', 'lint', 'format'))
